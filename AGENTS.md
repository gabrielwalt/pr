...
`PROJECT-*.md` files are the shared memory of the project — structured documents the agent reads every session and updates as the site evolves. They are human-readable too.

| File | Purpose | When to update |
|------|---------|------------------------|
| `PROJECT-DESIGN.md` | Design tokens, type scale, color, spacing, breakpoints, page templates, Home scroll interaction | When the global foundation or a template changes |
| `PROJECT-BLOCKS.md` | Block + variant + section-style inventory | Each time a block, variant, or section style is added or changed |
| `PROJECT-IMPORT.md` | Figma file link(s), node IDs per screen, frame-to-page mapping; the component library's location + DA/UE config state | When frame mappings or the library change |
