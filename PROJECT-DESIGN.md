...
| `contributors` | — | **new** (About; was `masthead`) | authored as a **native block table, one row per entry × 2 cells** (role \| name) — NOT a nested HTML `<table>`. JS builds a semantic `<table>` from those cells: each row → `<th scope="row">` key cell (`.contributors-key`, role, bold) + `<td>` value cell (`.contributors-value`, name, regular). CSS lays the table/rows/cells out as blocks (`display:block`) so it renders as the Figma **3-column masthead**: each entry stacks the bold role over the name, and entries flow into a `columns:3` multicol (top-to-bottom then across), `--space-xl` column gap, `--space-l` between entries, `--heading-font-size-s`/`--line-height-body-l` text; `break-inside:avoid` keeps role+name together. Mobile → 2 cols. | About (`/about`) | `/library/blocks/contributors` |
...
- **`template-content`** (`body.template-content`) — the About screen. Composes editorial content as **3 columns**: the contributors masthead (owned by the `contributors` block's own `columns:3` flow) and the issue archive (`main .projects.grid > ul { grid-template-columns: repeat(3, …) }`, owned by the template). About's `projects` block carries the `grid` variant (wrapping, not the scroller).
