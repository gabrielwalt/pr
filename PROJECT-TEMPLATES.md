...
- **`template-project-detail`** (`body.template-project-detail`) — 2026-07-09. The Project Detail long-form entry (Figma `14:10689`, wraps `Page Template` `14:11052`): `project-header` + 3-image overview + `event-list` + TOC (`project-table`) + long-form entry excerpts + `projects-search`. Same wrapper top/bottom-padding zeroing as `template-projects`. **Owns the DEFAULT-CONTENT styling for the entry excerpts** (the page blocks are styled by their own CSS; this template styles the non-block content): (1) overview — first section's leading `p` of images → flex row, equal `--space-s` gap, full-width; the following `p` → 32px lead. (2) entry sections (`main > .section > .default-content-wrapper`) — `h2` at 42px; the short grey byline via `h2 + p:not(:has(img))` (20px `--metadata-color`); cover image(s) via `p:has(img)` → centered flex, `max-height:70vh`, `object-fit:contain` (handles both the single cover and the Wind interview's side-by-side pair); body via `p:not(:has(img))` → 700px reading measure at 20px; **"Read more"** = the last `p`'s link → centered, bordered uppercase button (hover inverts). All scoped to the template body class → additive, no other page affected.
