...
See also: `edge-bleed-scroller` (the alternative bleed model: stay INSIDE the section's padded content column and cancel just that padding with symmetric negative margins + first/last item margin — use when the row should fill the *section column* edge-to-edge rather than escape past the *viewport* edge), `full-width-escape-hatch` (container constraint + full-width escape hatch), `eds-dom-structure` (wrapper chain). Native `building-blocks` covers the same ground at a generic level — this skill adds the `max()` left-margin formula and native scroll-snap approach for this project's Swiper-to-EDS migration.
