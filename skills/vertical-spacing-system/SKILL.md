...
- **Section-padding shows as a colored band when the section's surface color lives on the inner `> div`, not the `.section`.** The default `main > .section { padding: var(--section-padding) 0 }` puts vertical padding on the section, but if a page paints the *surface* (white sheet, hero image, dark ground) on the inner content `div` while the `.section` shows a different color (e.g. a black page ground behind a sticky/full-bleed neighbor), that `--section-padding` band renders in the section's color — a stray stripe above/below the surface. **If you're about to accept a thin colored gap between two full-bleed/edge-to-edge sections — stop; it's almost always this padding.** Fix: zero the offending side on that section (`padding-top: 0`, `padding-bottom: 0`, or `padding-block: 0` for full-bleed). This bit twice in one build: a grid sheet's `padding-top` showed black above a relocated nav, and a full-bleed hero's `padding-bottom` showed a black band before the next section rose over it. Verify with a seam probe: `section.getBoundingClientRect().bottom` vs the next section's `.top` should have gap 0 (or measure the inner `> div` bottom vs the section bottom to see the band).
...
See also: `eds-content-modeling` (where this fits in the augmented-styles ladder), `eds-dom-structure`, `full-width-escape-hatch`, `carousel-pattern-eds`, `eds-section-metadata-decoration` (the section-style variants here only apply if `decorateSections` converts Section Metadata — the enhanced boilerplate ships without that step). Native `building-blocks` covers the same ground at a generic level — this skill adds the two-token `* + *` pattern, the universal spacing variants, and the `overflow-x: clip` sticky fix.
